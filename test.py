...
for i in kejriwal.find({}):
        ...
        if "extended_tweet" in i:
                ...
                text_count+=1
        if "retweeted_status" in i:
                retweet_arvind_count+=i['retweeted_status']['retweet_count']

                favorite_count=favorite_count+i['retweeted_status']['favorite_count']

        if i['user']['time_zone'] not in arvind_tweet_locations and i['user']['time_zone']!="null":

                arvind_tweet_locations[i['user']['time_zone']]=1
        else:
                arvind_tweet_locations[i['user']['time_zone']]=arvind_tweet_locations[i['user']['time_zone']]+1
...
print(">>>>>>"+str(retweet_arvind_count)+"  "+str(tweet_arvind_count))
@app.route('/')
def hello_world():
    return render_template('index.html',tweet_arvind_count=tweet_arvind_count,tweet_modi_count=tweet_modi_count,
        tweets_Total=tweets_Total,retweet_total=retweet_total,favorite_count=favorite_count,m_location=json.dumps(modi_tweet_locations),trending=d,image_count=image_count,text_count=text_count,video_count=video_count)
